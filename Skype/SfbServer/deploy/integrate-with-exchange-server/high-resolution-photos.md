...
Uploading the photo (the first two commands in the example) doesn't equate to assigning that photo to Ken Myer's user account. Uploading the photo simply results in a preview of that photo being displayed on the Options page in Outlook on the web. To actually assign that photo to the user account, the user must select **Save** on the Options page or the administrator must run the third command in the example.

To verify that the new photo is assigned to the user account, Ken Myer can sign in Skype for Business, select **Options**, and then select **My Picture**. The newly uploaded photo should be displayed as Ken's personal photo. Alternatively, administrators can verify the photo for any user by opening a web browser to the following URL: `https://atl-mail-001.litwareinc.com/ews/Exchange.asmx/s/GetUserPhoto?email=kenmyer@litwareinc.com&size=HR648x648`.
...
No other configuration is required to make this photo available in Skype for Business. Instead, the photo will be instantly available after it is uploaded and the `Set-UserPhoto` cmdlet is run.
