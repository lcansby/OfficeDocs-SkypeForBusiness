...
This article provides an overview of the configuration requirements that are common to all three of these advanced Enterprise Voice features.
...
The same network region definitions are shared by all three advanced Enterprise Voice features. If you have already created network regions for one feature, you don't need to create new network regions for the other features. You might, however, need to modify an existing network region definition to apply feature-specific settings. For example, if you have created network regions for E9-1-1 (which don't require an associated central site) and, later, you deploy call admission control, you must modify each of the network region definitions to specify a central site.
...
> [!NOTE]
> Network sites are used only by the advanced Enterprise Voice features. They aren't the same as the branch sites that you configure in your Skype for Business Server topology.
...
For each network site, you'll need to work with your network administrator to determine which IP subnets are assigned to each network site. If your network administrator has already organized the IP subnets into network regions and network sites, then your work is simplified.
