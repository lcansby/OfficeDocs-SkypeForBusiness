...
- When using Azure Virtual Desktops / Windows 365 and MAC Remote Desktop Client or Windows app, Microsoft Teams UI will not enumerate all the peripherals you have connected to the device. Instead, only the device that is selected at the macOS-level is shown in the Devices' drop-down menu of Microsoft Teams. The peripheral is labeled as e.g. 'MAC device name' + "Speakers"
