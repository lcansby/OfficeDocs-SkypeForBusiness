...
3. On the **Trunk Configuration** page, use one of the following methods to configure a trunk:
   ...
   - **Site trunk**: Choose the site for this trunk configuration in **Select a Site**, and then click **OK**. Note that if a trunk configuration has already been created for a site, the site doesn't appear in **Select a Site**. This trunk configuration will be applied to all trunks in the site.

   - **Pool trunk**: Choose the name of the trunk that this trunk configuration applies to in **Select a Service** and click **OK**. This trunk can be the root trunk, or any additional trunks defined in Topology Builder. Note that if a trunk configuration has already been created for a specific trunk, the trunk doesn't appear in **Select a Service**.
...
6. Select the **Centralized media processing** check box if there's a well-known media termination point (for example, a Public Switched Telephone Network (PSTN) gateway where the media termination has the same IP as the signaling termination). Clear this check box if the trunk doesn't have a well-known media termination point.
...
17. When you're finished configuring the trunk, click **OK**.
