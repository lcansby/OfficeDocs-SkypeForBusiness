...
|**Unexpected failure volume** <br/> |No  <br/> |Total number of sessions where an "unexpected failure" occurred.  <br/> An unexpected failure is a failure that occurs in what would appear to be an otherwise healthy system. For example, a call shouldn't be terminated if the caller is placed on hold. If that occurs, that would be flagged as an unexpected failure.  <br/> |
