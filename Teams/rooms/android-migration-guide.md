...
> [!NOTE]
> An expired enrollment token will not impact any existing devices, just new device enrollments and sign-ins.
...
## Step 2 - Set up AOSP Management Configuration & Compliance Policies (if desired)
...
Currently, the only supported configuration policy for Teams Android Devices enrolled with AOSP Management is the Device Restrictions profile. Inside of this profile, various restrictions are supported, contact your Teams Android device OEM for specific supportability details.
...
7. Configure your desired and supported policies, then select **Next**.
...
As Team Android Device firmwares that support the migration to AOSP Management are released both for currently deployed devices and any new Teams devices, IT admins can install the firmware on their devices via Teams Admin Center.
