...
If Test-CsSetupPermission determines that the required permissions have already been set on an Active Directory container then the cmdlet returns the value True:

True 

If permissions aren't set, Test-CsSetupPermission will return the value False. This value will typically be enclosed in many warning messages. For example:
...
WARNING: The access control entries (ACEs) on the object "CN=Computers,DC=litwareinc,DC=com" aren't ready. 
