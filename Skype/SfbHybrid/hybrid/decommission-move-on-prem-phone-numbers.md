...
In the seamless migration method, the only action required to move a phone number from Skype for Businesss to online is to begin administering number assignments using the Teams admin center.

As administration tasks are performed on phone numbers in Teams, the *online service configuration* for those numbers takes precedence over the *on-premises Active Directory configuration*. By using Teams to assign the number to a user, the on-premises Active Directory configuration offered in every following Directory Sync is rejected.
