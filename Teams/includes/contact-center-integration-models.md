## Integration programs for solution providers

<a name="steps"></a>

As a contact center solution provider, there are three programs to choose from to integrate your connected contact center solution into Teams:

- If you want to use an SDK that enables solution providers to imbed native Teams experiences in their App, see the [Unify program](?tabs=unify#steps).

- If you want to use certified SBCs and Direct Routing to connect a contact center solution to Teams, see the [Connect program](?tabs=connect#steps).

- If you want to use Azure bots and the Microsoft Graph Communication APIs to enable solution providers to create Teams apps, see the [Extend program](?tabs=extend#steps).
...
The Unify program extends Teams phone system capabilities into CCaaS with the following resources:

- Azure Communication Services call automation
- Azure Communication calling SDKs
- OpenAI
...
While these features aren't a comprehensive list of feature capabilities for this program of integration, the focus areas include:
...
### [**The Extend Program**](#tab/extend)

The Extend program integrates with the Teams client using the [Teams client platform](/microsoftteams/platform/overview), [Teams Graph APIs](/graph/api/resources/teams-api-overview) and [Cloud Communications API in Microsoft Graph](/graph/api/resources/communications-api-overview). The Extend program also uses the Teams phone system for all contact center calls and call control experiences, and the contact center solution provider acts as a telephony carrier alongside Microsoft 365.
