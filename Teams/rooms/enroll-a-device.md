...
> [!IMPORTANT]
> Both automatic and manual deployment require onboarding Microsoft Teams Rooms (MTR) devices to the MTR Pro Management portal (PMP). The monitoring service agent is intended for use with certified MTR systems and peripherals.
...
1. Run the following command (double quotes at end of command are important):
   ...
   - If using a ***pac*** file: `bitsadmin /Util /SetIEProxy LOCALSYSTEM MANUAL_PROXY <pac file url>`
      
      *Example:*
      
      ```cmd
      bitsadmin /Util /SetIEProxy LOCALSYSTEM MANUAL_PROXY http://contosoproxy.corp.net/proxy.pac
      ```
...
1. In the Microsoft Teams Rooms user interface, select __More__, then select __Settings__, where you're prompted for the local Administrator password on the device.

1. Select __Settings__, then select __Windows Settings__ to access Windows as local administrator.

1. From the list of users displayed in the Windows login screen, select __Administrator__ (or the respective local administrator of your device).
...
## URLs Required for Communication
 ...
 > [!NOTE]
 > All network traffic between the Teams Rooms devices agent and the Microsoft Teams Rooms Pro Management portal is SSL over port 443*.*  See [Teams Rooms - Security](security.md?tabs=Windows#network-security) for the full list of required connectivity endpoints for functionality.
...
To expedite the automatic enrollment in the Teams Rooms Pro management portal, consider restarting the Teams Rooms device after a few minutes following the agent deployment.
...
***Application runtime rolling log file location*** =
