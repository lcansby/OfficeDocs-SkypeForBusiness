...
f1.keywords:
- NOCSH
description: Learn about feature functionality, customer scenarios, and solutions for Local Media Optimization for Direct Routing
appliesto:
  - Microsoft Teams
---
...
This article describes feature functionality, customer scenarios, and solutions. For details on configuration, see [Configure Local Media Optimization](direct-routing-media-optimization-configure.md).
...
For this discussion, assume that Contoso runs multiple businesses across the globe as follows. (Europe and APAC regions are used as examples only. A company might have several different regions with similar requirements.)
...
Afterwards, the Contoso administrator adds some downstream SBCs indicating that they can be reached through the proxy SBC proxysbc.contoso.com. Downstream SBCs don't have public IPs; however, they can be assigned to voice routes. The following table shows example network parameters and configuration.
...
- The downstream SBC in the local branch office isn't visible to Teams Phone directly but is mapped within the virtual network topology that's defined by the Contoso administrator while setting up Local Media Optimization.
...
- The downstream SBC in the local branch office isn't visible to Teams Phone directly, but is mapped within the virtual network topology that's defined by the Contoso administrator while setting up Local Media Optimization. In the example, the user is considered external because the user is outside the boundaries of the corporate network.
...
> [!NOTE]
> When users are internal, media connectivity between the user and the SBC over the internal IP address is **required**. In this case, there's no fallback to public transport relays for media as the SBC provides an internal IP for media connectivity.
...
The SBC in Germany can be configured in "Always bypass" mode. Users, regardless of their location, connect to the SBC directly through the internal IP address of the SBC--for example, from France to Germany. See the following diagram for reference.
...
The following list of behaviors might be observed when using Local Media Optimization:

| Product behaviors | Notes |
| :--- | :--- |
| Teams client isn't identified as **internal** when the Teams client public IP matches the customer trusted IP list, but doesn't match the network site. | Local Media Optimization requires a matching [network site](/powershell/module/teams/new-cstenantnetworksubnet) for every connection made from a listed trusted IP. |
| Teams user puts the call On Hold. Music plays on the PSTN end and Local Media Optimization is working. The Teams user resumes the call. The call to PSTN resumes but Local Media Optimization isn't working and the call continues via Central (Proxy) SBC. | When a user parks a call to initiate music on hold (MoH), the call escalates from a 1:1 call to a multiparty call by the Call Controller to invoke Media Controller and Media Processor (serving as AVMCU mixer) through which MoH reaches a user who has been put on hold. De-escalation to a 1:1 call after the call resumes never happens, as per design. |
| While a call is being established, the user might hear silence for a few seconds. | This silence might occur in some cases due to the complexity of Local Media Optimization architecture. |
| Voice apps (for example, Auto Attendant and Call Queue). | Voice apps can be deployed in an environment with LMO configured. However, calls involving voice apps aren't optimized for Local Media Optimization. Instead, they flow through the proxy SBC, except for transfers from Auto Attendant to Local Media Optimization users. In such cases, the call follows the Local Media Optimization optimized path, as it's a direct 1:1 call. For Location-Based Routing scenarios, see [Voice apps (Auto Attendant or Call Queue)](location-based-routing-plan.md). |

## Related articles

[Configure Local Media Optimization](direct-routing-media-optimization-configure.md)

[Plan Media Bypass](direct-routing-plan-media-bypass.md)
