...
1. On the **Routing rules** tab, select **Add**.
...
1. On the **Numbers** tab, select an unassigned phone number and select **Route** at the top of the view

1. Give the rule a name, a description, and specify the evaluation order for the rule.

1. Select the routing option.
...
The next example specifies that all calls to the number range +1 (555) 333-0000 to +1 (555) 333-9999 are routed to the announcement service, which then plays the audio file MainAnnouncement.wav to the caller.
...
- If routing to an announcement, the audio file is played once to the caller.
