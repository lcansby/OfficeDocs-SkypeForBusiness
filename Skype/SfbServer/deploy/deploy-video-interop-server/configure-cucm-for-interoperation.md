...
Several CUCM settings need to be confirmed or changed for interoperation with the VIS. Follow the procedures below in order to avoid missing required settings.
  
### Configure the CUCM

1. Sign in to CUCM and navigate to Cisco Unified CM Administration-\>Call Routing-\>Class of Control-\>Partition.
    
2. In the Partition Configuration screen, enter the partition name and description and select on **Add New**.
    
3. Navigate to Cisco Unified CM Administration-\>Call Routing-\>Class of Control-\>Calling Search Space.
    
4. In the Calling Search Space Configuration screen, enter the name for the calling search space, and in Selected Partitions, enter the name of the partition you created. Select **Save** when done.
    
5. Navigate to Cisco Unified CM Administration-\>System-\>Security-\>SIP Trunk Security Profile.
    
6. In the SIP Trunk Security Profile Configuration screen, set the SIP Trunk Security Profile Information options as shown, and select on **Add New**.
...
12. When you're finished, select on **Add New**.
...
16. Scroll down further. Media Termination Point (MTP) isn't required for Video Calls, if it isn't already unchecked, uncheck it. Check the option to **Run on all active Unified CM Nodes**. You should add all CUCM nodes to the Skype for Business Server configuration.
...
20. When you're finished, select on **Add New**.
    
21. Navigate to Cisco Unified CM Administration-\>Call Routing-\>Route/Hunt-\>Route pattern.
    
22. In the Route Pattern Configuration screen, enter the Pattern definition parameters shown below. Scroll down to the Called Party Transformations section and set the mask as shown, and then select on **Add New** when finished.
...
24. In the SIP Route Pattern Configuration screen, set the Pattern Definition options as shown, and select on **Add New**.
...
25. If you change the audio or video bit rates from the default settings, you need to return them to the defaults. To set the bit rate for Audio/Video calls, navigate to Cisco Unified CM Administration-\>System-\>Region Information-\>Region. The defaults are shown below for reference:
    ...
    |Maximum Session Bit Rate for Video Calls  <br/> |200,000 kbps  <br/> |
    |Maximum Session Bit Rate  <br/> |2,000,000,000 kbps  <br/> |
   
At this point the CUCM video gateway, is configured to work with the VIS. Corresponding configuration needs to be done on each VTC you wish to integrate.
> [!NOTE]
> To improve resiliency, you may want to configure this CUCM gateway to work with a second Video Interop Server or VIS pool. For more information, see [Resiliency mechanisms](../../plan-your-deployment/video-interop-server.md#resiliency).
