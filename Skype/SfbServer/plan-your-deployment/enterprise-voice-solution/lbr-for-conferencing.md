...
- If organizers and participants are joining the meeting from different or unknown network sites, a PSTN endpoint isn't allowed to join the meeting if the PSTN call ingresses from a SIP trunk enabled for Location-Based Routing.

- If organizers and participants are all joining the meeting from the same network site and there are participants joining the same meeting from the PSTN, a Skype for Business endpoint from a different network site isn't allowed to join the meeting.
...
- When a user isn't allowed to join a conference given Location-Based Routing restrictions, the calls to the conference are rejected and the Skype for Business client will report that the call wasn't completed or has ended.

- A PSTN endpoint joining a conference with Location-Based Routing enforcements won't be restricted to join the conference regardless of its state if the endpoint joins via a trunk that isn't enabled for Location-Based Routing.

- A PBX system connected to a Mediation Server over a SIP trunk that doesn't egress calls to the PSTN will have the same enforcements as Skype for Business users located in the same network site where the SIP trunk is defined. For example, a PSTN endpoint is able to join a conference with a PBX user and a Skype for Business user if they're located in the same network site; otherwise, the PSTN endpoint won't be allowed to join the conference if the PBX user is in a different network site than the Skype for Business user.
...
- If the SIP trunk routing the PSTN call is authorized to re-route the PSTN calls to the network site where Skype for Business user B (that is, transfer target) is located, then the call transfer is allowed; otherwise, the consultative call transfer is blocked. This authorization is performed based on the transferred party's location being in the same network site as the SIP trunk that is routing the active call to the PSTN endpoint.

- If the SIP trunk routing the inbound PSTN call isn't authorized to route calls to the network site where the transferred party (Skype for Business user B) is located or the transferred party is located in an unknown network site, then the consultative call transfer to the PSTN endpoint (that is, call transfer target) are blocked.
...
|:-----|:-----|:-----|
|PSTN endpoint  <br/> |Skype for Business user in the same network site (that is, site 1)  <br/> |Consultative transfer is allowed  <br/> |
|PSTN endpoint  <br/> |Skype for Business user in different network sites (that is, site 2)  <br/> |Consultative transfer is disallowed  <br/> |
|PSTN endpoint  <br/> |Skype for Business user in an unknown network site  <br/> |Consultative transfer is disallowed  <br/> |
|PSTN endpoint  <br/> |Federated Skype for Business user  <br/> |Consultative transfer is disallowed  <br/> |
|PSTN endpoint  <br/> |PBX endpoint in the same site (that is, site 1)  <br/> |Consultative transfer is allowed  <br/> |
|PSTN endpoint  <br/> |PBX endpoint in a different sites (that is, site 2)  <br/> |Consultative transfer is disallowed  <br/> |
|PBX endpoint in the same site (that is, site 1)  <br/> |PSTN endpoint  <br/> |Consultative transfer is allowed  <br/> |
|PBX endpoint in a different site (that is, site 2)  <br/> |PSTN endpoint  <br/> |Consultative transfer is disallowed  <br/> |
|PBX endpoint in any site  <br/> |Skype for Business user in the same network site (that is, site 1)  <br/> |Consultative transfer is allowed  <br/> |
|PBX endpoint in any site  <br/> |Skype for Business user in different network sites (that is, site 2)  <br/> |Consultative transfer is allowed  <br/> |
|PBX endpoint in any site  <br/> |Skype for Business user in an unknown network site  <br/> |Consultative transfer is allowed  <br/> |
|PBX endpoint in any site  <br/> |Federated Skype for Business user  <br/> |Consultative transfer is allowed  <br/> |

## Requirements

The Location-Based Routing for Conferencing application requires that either Skype for Business Server or Lync Server 2013 Cumulative Update 2 is deployed on all Front-End pools and Standard Edition Servers in your topology. If these server versions are not installed on some servers in your topology, Location-Based Routing restrictions can't be fully enforced on meetings and consultative call transfers.
