...
3.  In the left navigation bar, selects **Network Configuration**, and then click **Global**.

4.  On the **Global** page, select the **Global** configuration. There's always only one configuration, and it's always named Global.

5.  On the **Edit** menu, select **View details**.

6.  On the **Edit Global Setting** page, click the **Enable media bypass** check box.

7.  Select one of the following options:
    
      - **Always bypass**   Select this option to attempt media bypass on all calls. This option will be unavailable if call admission control (CAC) is enabled. If CAC isn't enabled, select this option in the following situations:
        
          - There's no need for bandwidth control.
        
          - There's no need for fine-grained configuration to determine when bypass should happen.
        
          - There's full connectivity between gateways and clients.
    
      - **Use sites and region configuration**   If CAC is enabled, this option is selected by default and can't be changed. When this option is selected, network configuration sites and regions are used to determine when media bypass is possible. If you select this option, you can choose to enable bypass for sites that aren't mapped. Select the **Enable bypass for non-mapped sites** check box only if you have one or more large sites associated with the same region that don't have bandwidth constraints (for example, a large central site) and you also have some branch sites associated with the same region that do have bandwidth constraints. When you enable bypass for non-mapped sites, configuration is streamlined because you specify only the subnets associated with the branch sites rather than needing to specify all subnets associated with all sites. We recommend that you do not select the **Enable bypass for non-mapped sites** check box if CAC is enabled.
