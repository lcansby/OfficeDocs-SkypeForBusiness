...
It's important to note that while all these features are available once Mobility is implemented, they might work a little differently on some devices. We've got a website that discusses what features work on what devices, at [Mobile client feature comparison for Skype for Business](clients-and-devices/mobile-feature-comparison.md). We also have some great device and OS information at [Plan for clients and devices](clients-and-devices/clients-and-devices.md).
  
Mobility makes use of the Autodiscover feature, which allows clients to automatically locate Skype for Business Server web services without users needing to enter in any URLs (they don't even need to know them). If you need to do some troubleshooting, manual entry of URLs is still supported.
...
The Mobility service is supported on Front End Servers when collocated with the Mediation Server role, with two network interfaces, but you need to take appropriate steps to configure those interfaces. You need to assign IP addresses to the specific interface that communicates as the Mediation Server, and the network IP interface that communicates as the Front End Server. You can do this in Topology Builder by selecting the correct IP address for each service, instead of using the default **Use all configured IP addresses** selection.
  
## Technical requirements
<a name="TechRequirements"> </a>

It's important to plan for the various mobile application scenarios your mobile users might encounter. For example, someone might start using a mobile app outside of work by connecting through a 3G network, then switch to the corporate Wi-Fi network when they reach work. They might switch to 4G when leaving their building. Planning now allows you to support these network transitions and guarantee a consistent user experience.
  
### DNS configuration

The Mobility services Mcx and UCWA use DNS in the same way. With Automatic Discovery, mobile devices use DNS to locate resources. During DNS lookup, a connection's attempted to the FQDN that's associated with the internal DNS record (lyncdiscoverinternal.[internal domain name]). If the internal DNS record can't be used to make that connection, a second connection is attempted, this time to the external DNS record (lyncdiscover.[sipdomain]). So why have two? A mobile device that's internal to your network will be able to use the internal Autodiscover URL. External mobile devices will use the external Autodiscover URL. In either case, the Autodiscover service returns all Web service URLs for the user's home pool, which includes the Mobility service (Mcx and UCWA).
...
> [!NOTE]
> As we just noted, all Mobility service traffic (internal and external) goes through your reverse proxy. But sometimes an issue comes up when the internal traffic leaves through an interface, only to then try and come back in on the same interface. This can violate your spoofing (formally it's called TCP packet spoofing) security rules. You need to allow **Hair Pinning** to have Mobility function.
...
There are some things you need to configure to support users inside and outside your corporate network.
...
If you support automatic discovery, you need to make the following DNS records for each SIP domain:
...
The internal automatic discovery URL shouldn't be addressable from outside your internal network. The external automatic discovery URL shouldn't be addressable from within your network. But if this isn't possible for the external URL, your mobile client functionality probably isn't affected, because the internal URL is always tried first.
...
If you're using automatic discovery for your Skype for Business mobile clients, you need to modify the SAN (subject alternative name) lists on your certificates to support secure connections from your mobile clients. If you already have certificates in-place, you need to request and assign new certificates with the SAN entries described here. This needs to be done for each Front End Server and Director (if in your environment) that runs the Autodiscover service. We'd also recommend modifying the SAN lists on your reverse proxy certificates, adding SAN entries for every SIP domain in your organization.
  
This should be a straightforward process if you're requesting the new certs off an internal CA (certificate authority), but public certificates are more complex, and potentially a lot more expensive to re-request, not to mention it might be costly to add many SIP domains to a new public cert. In that situation, there's an approach that's supported, but **not recommended**. You can configure your reverse proxy to make the initial Autodiscover service request over port 80, which will use HTTP, rather than port 443, which is HTTPS (and 443 is the default configuration). That incoming request will be redirected to port 8080 on your Front End pool or Director. By doing this, you won't need to make any certificate changes, because this traffic isn't using HTTPS for requests. But again, we don't recommend this, although it works for you.
  
### Windows and IIS requirements

You should have a supported Windows Server version for your Skype for Business Server environment. As a result, you should also have IIS 8 or IIS 8.5 for your mobility needs. There needs to be some changes to the default ASP.NET settings, but the Mobility service installer does that automatically.
...
For details on all this, review the [Load balancing requirements for Skype for Business](network-requirements/load-balancing.md) documentation.
  
### Reverse Proxy requirements

In order to support automatic discovery for Skype for Business mobile clients, you need to update the current publishing rule as follows:
...
- If you've decided to use HTTP for the initial Autodiscover service request to avoid having to update the SAN list for your reverse proxy certificates (which we don't recommend), you need to create a new web publishing rule for port HTTP/TCP 80, if there isn't one already. If that rule exists, update it to include a lyncdiscover.\<sipdomain\> entry.
    
## Defining your Mobility needs
<a name="MobilityNeeds"> </a>

Now that we've reviewed the topologies, components and technical requirements, let's look at what your organization might need in terms of a Mobility implementation.
  
### Do you want to use automatic discovery for Skype for Business mobile clients?

We do strongly recommend that you do use automatic discovery. It requires the creation of new internal and external DNS records, as documented in the Technical Requirements section above. With automatic discovery, the Skype for Business clients can automatically locate Skype for Business Server Web Services from any location, without needing a URL to be entered in manually.
  
You can use manual settings if you need to. These URLs need to be entered by users into their mobile devices:
...
Again, we do recommend using automatic discovery. You might find manual settings useful for troubleshooting purposes.
  
### Are you going to support push notifications?

Push notifications are used for mobile applications that support this functionality to notify a user of events while the app's not active. Your Edge Server needs to have a federation relationship with your cloud-based Skype for Business Server Push Notification Service, which is found on the Skype for Business Online datacenter. You need to run a cmdlet to enable push notifications.
  
> [!NOTE]
> If you have anyone still using Lync Server 2010 clients, they need TCP port 5223 open outbound on your enterprise WiFi network. 
...
We have a table to help with some of the features that are available to all users, and whether they're set that way or not by default. For a complete list, review [New-CsMobilityPolicy](/powershell/module/skype/new-csmobilitypolicy?view=skype-ps&preserve-view=true).
...
|:-----|:-----|:-----|:-----|
|Enable Mobility  <br/> |EnableMobility  <br/> |Controls users in a given scope who have Skype for Business mobile client installed. If the policy is set to False, your users can't sign in with their client.  <br/> |True  <br/> |
|Outside Voice  <br/> |EnableOutsideVoice  <br/> |Enables a user's ability to use Call Via Work, which lets users send and receive calls by using their work number instead of their mobile number. If it's set to False, your users can't make or receive calls on their mobile phone when using their work phone number.  <br/> |True  <br/> |
|Enable IP Audio and Video  <br/> |EnableIPAudioVideo  <br/> |Set to the default, it allows a user to use VoIP to make or receive phone or video calls on their mobile device. When set to False, your users can't use their mobile device to do either of those things.  <br/> |True  <br/> |
|Require WiFi for IP Audio  <br/> |RequireWiFiForIPAudio  <br/> |Defines whether a client will need to make and receive calls over VoIP on WiFi instead of a cellular data network. If it's set to True, your users can only make and receive VoIP calls when they're connected via WiFi.  <br/> |False  <br/> |
|Require WiFi for IP Video  <br/> |RequireWiFiForIPVideo  <br/> |Defines whether a client needs to make and receive video calls on WiFi instead of a cellular data network. If it's set to True, your users can only make and receive VoIP calls when they're connected via WiFi.  <br/> |False  <br/> |
...
Either way, the Voice policy you assign needs to have public switched telephone network (PSTN) usage records and routes that will define where your users are able to dial out to join conferences.
