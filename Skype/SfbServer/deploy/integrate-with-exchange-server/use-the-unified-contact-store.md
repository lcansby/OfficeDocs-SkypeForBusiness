...
After the policy is assigned, Skype for Business Server will begin to migrate the user's contacts to the unified contact store. After migration is complete, the user will then have their contacts stored in Exchange rather than Skype for Business Server. If the user happens to be logged on to Lync 2013 at the time migration completes, a message box appears and they'll be asked to sign out of Skype for Business and then log back on in order to finalize the process. Users who are not assigned this per-user policy won't have their contacts migrated to the unified contact store. That's because those users are managed by the global policy, and use of the unified contact store is disabled in the global policy.
