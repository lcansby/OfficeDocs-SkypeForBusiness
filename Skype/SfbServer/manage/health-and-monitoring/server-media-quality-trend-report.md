...
When you select the Trend metric on the [Server Performance Report in Skype for Business Server](server-performance.md) for a specific server, the Server Media Quality Trend Report opens. However, you see only a blank instance of that report; the server you selected on the Server Performance Report won't be displayed onscreen. Instead, you need to select that server from the Servers dropdown. Note, too that the Servers dropdown includes a Select All option. This option won't work if you have more than five servers; the Server Media Quality Trend Report can only display data for a maximum of five servers at a time.
