---
title: Use Windows Autopilot and Autologin to easily deploy Microsoft Teams Rooms consoles
author: mstonysmith
ms.author: tonysmit
manager: pamgreen
ms.reviewer: altsou
...
description: Learn how to set up and use the Windows Autopilot and Autologin features to deploy and provisioning Microsoft Teams Rooms on Windows consoles in your organization.
---
...
To use Windows Autopilot and Autologin to provision and deploy Microsoft Teams Rooms consoles in your organization, you must perform these tasks:

- Step 1: [Verify that all prerequisites are met](#step-1-prerequisites)
- Step 2: [Register devices as Windows Autopilot devices](#step-2-registering-teams-rooms-consoles-using-windows-autopilot)
...
- Step 5: [Create an Enrollment Status Page (ESP) profile](#step-5-create-a-windows-autopilot-enrollment-status-page-esp-status-page-profile)
- Step 6: [Create and assign a Windows Autopilot profile](#step-6-create-a-windows-autopilot-profile)
...
1. You must buy enough Teams Rooms Pro licenses for each of the consoles you're provisioning and deploying. Teams Rooms Pro licenses include the correct Intune and Microsoft Entra ID P1 licenses. To see more information on the licensing requirements for Windows Autopilot, see [Windows Autopilot licensing requirements](/autopilot/requirements?tabs=licensing#licensing-requirements).
...
4. You must join Microsoft Entra devices. Windows Autopilot self-deploying mode is not supported for Microsoft Entra hybrid join devices.
...
> [!Note]
> To test Windows Autopilot, you can manually register Windows Autopilot devices in Intune. You can do this several different ways, see [Manually register devices with Windows Autopilot](/autopilot/add-devices).
...
To create a dynamic device group that includes all of the Teams Rooms consoles to use Windows Autopilot, use the following query:
...
The Teams Rooms app update tool updates the Teams room app running on the device to a version that supports Windows Autopilot and Autologin. The update tool needs to be first downloaded, then uploaded to Intune, and deployed to the Teams Rooms on Windows consoles. Although it's not required, use dynamic device groups. During the Windows Autopilot Enrollment Status Page (ESP), Intune installs the update tool to the Teams Rooms console and updates the Teams room app before it starts.
...
## Step 5: Create a Windows Autopilot Enrollment Status Page (ESP) status page profile

To create an enrollment status page profile for your Teams Room on Windows consoles, see [Configure and assign the Enrollment Status Page (ESP)](/autopilot/tutorial/self-deploying/self-deploying-esp).
...
## Step 6: Create a Windows Autopilot profile

For the Teams Rooms consoles, you must create a Self-deploying Windows Autopilot profile. See [Create and assign self-deploying Windows Autopilot profile](/autopilot/tutorial/self-deploying/self-deploying-autopilot-profile).

Then assign the Windows Autopilot profile to the previously created device group in [Step 3](#step-3-create-a-group-for-the-teams-rooms-consoles).
...
After the Endpoint Manager portal configuration is complete, you must assign the resource accounts for the consoles listed as Windows Autopilot devices that let the Teams Rooms consoles automatically log in when they're deployed.
...
To assign an account to a Windows Autopilot device, the device must have a Windows Autopilot profile assigned. You can see the devices in the **Profile assignment status** column and they should be listed as **Assigned**.
...
6. On the **Review** page, select **Finish** to link the resource account to Windows Autopilot device.
...
To start the Windows Autopilot deployment process on the console that is Windows Autopilot registered and has a resource account assigned.
...
3. Once the device boots up, one of two things occurs depending on the state of network connectivity:
   ...
   - If the device is connected to a wired network, the device may reboot to apply critical security updates if they're available. After the reboot to apply critical security updates, the Windows Autopilot process begins.
   ...
   1. The device may reboot to apply critical security updates if they're available. After the reboot to apply critical security updates, the Windows Autopilot process begins.
...
6. The Teams Room app now detects the Windows Autopilot profile and initiates Autologin. The credentials for the resource account assigned to this Windows Autopilot device are used. When this part is complete, the console will automatically log in and is ready for Teams meetings.

## Frequently asked questions

**Question** Why aren't my Windows Autopilot devices syncing into the Teams Room Pro Management portal?

**Answer** In the Teams Pro Management portal on the Windows Autopilot device page, select **Sync** to initiate an update to the device list. Check the "Last synced" time on the page to see if it corresponds to the time the last sync was initiated. If devices are still not syncing, check that the GroupTag is configured correctly with the prefix **MTR-ConsoleName**.
...
**Question** Can I use Windows Autopilot to EntraID join the device into one tenant and manually log in the Teams Room app to a resource account from another tenant?
