...
When troubleshooting the new Slimcore-based optimziation for Microsoft Teams, you need to know if users are optimized with the legacy WebRTC stack. They may also have ended up in the fallback mode "SlimCore Media Not Connected" (or server side rendering).
